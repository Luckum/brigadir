...
(function($){
	 ...
	 *      onShow      Callback when Floater created
	 *
	 *
	 * @version 1.4.0
    * @see http://blog.slackers.se/2009/07/jquery-floating-table-header-plugin.html
	...
	$.fn.floatHeader = function(config) {
		...
		return this.each(function () {	
			...
			$(tableClone).css('margin-left', 0); // Исправление для широких таблиц
			...
			scrollElement.unbind('scroll.' + $(this).attr('id')).bind('scroll.' + $(this).attr('id'), function() {	
	...
	function showHeader(element, floater) {
		...
		if (element.height() != 0)  {
			var floatHeight = 0;
			if (self.floatBoxVisible)
				floatHeight = floater.height();
			
			var height = element.height() - floatHeight;

...
function nc_subdivision_form_fields_save() {
    ...
    $nc_core->event->execute("updateSubdivisionPrep", $site_id, $sub_id);
    $db->query($update);
    $nc_core->event->execute("updateSubdivision", $site_id, $sub_id);

    // перезагрузить данные, чтобы иметь в nc_core->subdivision актуальную версию...
    $nc_core->subdivision->get_by_id($sub_id, null, true);

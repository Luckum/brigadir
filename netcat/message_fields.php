        ...
        for ($i = 0; $i < $fldCount; $i++) {
            ...
            if ($fldInheritance[$i] && $message && $systemTableID) {
                switch ($nc_core->get_system_table_name_by_id($systemTableID)) {
                    case 'Template':
                        ...
                        $nc_field_has_inherited_value = $nc_parent_id && strlen($nc_core->template->get_by_id($nc_parent_id, $fld[$i]));
                        break;
                    case 'Subdivision':
                        $nc_parent_id = $nc_core->subdivision->get_by_id($message, 'Parent_Sub_ID');
                        $nc_field_has_inherited_value = strlen(
                            $nc_parent_id
            ...
            switch ($fldType[$i]) {
                ...
                case NC_FIELDTYPE_MULTIFILE:
                    ...
                    $not_null = $nc_field_requires_value && count($_FILES["f_{$fld[$i]}_file"]['name']) == 0;

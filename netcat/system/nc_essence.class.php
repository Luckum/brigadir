            ...
            if ($env_array[$field_name]) {
                switch ($field_type) {
                    ...
                    case NC_FIELDTYPE_FILE:
                        ...
                        $files_folder = $nc_core->get_variable("SUB_FOLDER") . $nc_core->get_variable("HTTP_FILES_PATH");

                        if ($row) {
                            // Protected FileSystem
                            $env_array[$field_name] = $files_folder . ltrim($row[1], '/') . "h_" . $row[0];
                            $env_array[$field_name . "_url"] = $files_folder . ltrim($row[1], '/') . $row[0];
                            $env_array[$field_name . "_preview_url"] = $files_folder . ltrim($row[1], '/') . "preview_" . $row[0];
                        } else {
                            if ($file_data[3]) {
                                // Original FileSystem
                                $env_array[$field_name] = $env_array[$field_name . "_url"] = $files_folder . $file_data[3];
                                $env_array[$field_name . "_preview_url"] = $files_folder . dirname($file_data[3]) . "/preview_" . basename($file_data[3]);
                            } else {
                                // Simple FileSystem
                                $env_array[$field_name] = $env_array[$field_name . "_url"] = $files_folder . $field_id . "_" . $env_array[$this->essence . "_ID"] . $ext;
                                $env_array[$field_name . "_preview_url"] = $files_folder . 'preview_' . $field_id . "_" . $env_array[$this->essence . "_ID"] . $ext;
                            }
                        }

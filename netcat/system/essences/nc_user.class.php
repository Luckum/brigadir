...
class nc_User extends nc_Essence {
    ...
    public function get_by_id($id, $item = "", $reset = false) {
        //global $current_user; // <<-- deprecated
        $id = intval($id);
    ...
    public function add($fields, $groups, $password, $add_fields = array(), $registration_code = null) {
        ...
        $this->core->event->execute('addUserPrep', 0);
        ...
        $this->core->event->execute('addUser', $user_id);
    ...
    public function authorize_by_pass($login, $password, $captcha = null) {
        ...
        if (nc_module_check_by_keyword('auth')) {
            ...
            if (nc_auth::get_object()->need_captcha() && !nc_captcha_verify_code($captcha)) {
                if ($captcha !== null) {
                    // каптча была передана
                    nc_auth::get_object()->set_invalid_captcha();
        ...
        $user_result = $this->db->get_results("SELECT `u`.User_ID AS `User_ID`, `Password`,
              ...
              WHERE
              `u`.`" . $this->core->AUTHORIZE_BY . "` = '" . $this->db->escape($login) . "'
              AND `Password` = " . $this->core->MYSQL_ENCRYPT . "('" . $this->db->escape($password) . "')
              AND `Checked` = 1
    ...
    protected function update_user_login_counter($login, $increment = 0) {
        $nc_core = nc_core::get_object();
        if ($nc_core->get_settings('auth_captcha_num', 'auth')) {
            $db = $nc_core->db;
            $db->query(
                "UPDATE `User`
